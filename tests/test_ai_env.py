...
def test_hvac_env_initialization():
    ...
    obs, info = env.reset()
    assert len(obs) == 7 # t_out, t_in, co2, pm, time_sin, time_cos, occ

def test_reward_calculation_extreme_co2():
    """Тестування нарахування штрафів за високий рівень CO2."""
    env = HVACEnv(db_path="data/recuperator_db.json")
    
    # Фіктивний стан: ідеальна температура, але критичний CO2
    obs_dict_bad = {"temp": 22.5, "co2": 2500.0, "pm": 10.0, "energy": 50.0}
    ...
    assert reward_bad < -2.0 # Штраф за 2500 ppm CO2 має бути відчутним
...
def test_normalization_bounds():
    ...
    # Задаємо екстремальні значення, але час має бути в межах [-1, 1]
    env.state = np.array([-50.0, 50.0, 10000.0, 500.0, 1.0, -1.0, 100.0])
    obs = env._get_obs()
    
    # Перевіряємо, що np.clip() відпрацював
    assert np.all(obs >= -1.0) # Часові синуси можуть бути від -1 до 1
