...
def init_session_state():
    ...
    if "project_settings" not in st.session_state:
        st.session_state.project_settings = {
            ...
            "wall_thickness": 0.38,
            "wall_type_0": "Зовнішня", "wall_mat_0": "Базовий",
            "wall_type_1": "Внутрішня", "wall_mat_1": "Базовий",
            "wall_type_2": "Внутрішня", "wall_mat_2": "Базовий",
            "wall_type_3": "Внутрішня", "wall_mat_3": "Базовий",
            "wall_type_4": "Зовнішня", "wall_mat_4": "Базовий",
            "wall_type_5": "Зовнішня", "wall_mat_5": "Базовий",
...
def add_element(el_type, room_l, room_w):
    ...
    new_row = pd.DataFrame([{
        "Тип": el_type, "X": room_l / 2, "Y": 0.0,
        "Ширина": 1.2 if el_type == "Вікно" else 0.8,
        "Орієнтація": "Горизонтально"
    }])
    st.session_state.elements_df = pd.concat([st.session_state.elements_df, new_row], ignore_index=True)
...
def render_blueprint(l, w, l_cut, w_cut, elements):
    fig = go.Figure()
    ...
    fig.add_shape(
        ...
        line=dict(width=0), fillcolor="rgba(223, 230, 233, 0.2)"
    )
    ...
    for i in range(6):
        ...
        fig.add_shape(
            type="line", x0=x_verts[i], y0=y_verts[i], x1=x_verts[i+1], y1=y_verts[i+1],
        ...
        fig.add_annotation(
            x=mid_x + offset_x, y=mid_y + offset_y, text=wall_labels[i],
            showarrow=False, font=dict(size=14, color="gray"), textangle=textangle
        )

    for i, row in elements.iterrows():
        if row["Тип"] == "Вікно": color = "#00a8ff"
        elif row["Тип"] == "Джерело тепла": color = "#e84118"
        else: color = "#f39c12"
        size = row["Ширина"]
        ...
        fig.add_shape(
            type="rect", x0=row["X"] - w_h, y0=row["Y"] - h_h,
            x1=row["X"] + w_h, y1=row["Y"] + h_h,
            line=dict(color=color, width=2), fillcolor=color
        )
        fig.add_annotation(
            x=row["X"], y=row["Y"], text=f"{row['Тип']} {i+1}",
            showarrow=False, font=dict(size=14, color="black")
        )
    fig.update_xaxes(
        range=[-1.0, l+1.0], showgrid=True, gridcolor='#dcdde1', 
        dtick=1.0, tickfont=dict(size=14), title_font=dict(size=16)
    )
    fig.update_yaxes(
        range=[-1.0, w+1.0], showgrid=True, gridcolor='#dcdde1', 
        dtick=1.0, tickfont=dict(size=14), scaleanchor="x", scaleratio=1
    )
    fig.update_layout(template="plotly_white", height=600, margin=dict(l=20, r=20, t=50, b=20))
    return fig

def export_project(room_l, room_w, room_h, room_l_cut, room_w_cut):
    project = {
        "geometry": {"L": room_l, "W": room_w, "H": room_h, "L_cut": room_l_cut, "W_cut": room_w_cut},
