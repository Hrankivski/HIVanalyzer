"""
Модуль валідації симуляційних даних.
...
class ModelValidator:
    ...
    def test_hx_efficiency(self, df):
        ...
        if (
            "T_supply (C)" in df.columns
            and "T_out (C)" in df.columns
            and "T_in (C)" in df.columns
        ):
            ...
            if not df_cold.empty:
                ...
                if not df_valid.empty:
                    df_valid["Efficiency"] = (
                        df_valid["T_supply (C)"] - df_valid["T_out (C)"]
                    ) / df_valid["Denominator"]
    ...
    def test_contaminant_decay(self, df):
        ...
        if "Generic Contaminant" in df.columns and "Datetime" in df.columns:
            ...
            if not df_16.empty and not df_17.empty:
                ...
                if not merged.empty:
                    merged["Ratio"] = (
                        merged["Generic Contaminant_17"]
                        / merged["Generic Contaminant_16"]
                    )
