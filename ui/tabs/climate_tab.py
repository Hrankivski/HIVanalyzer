...
def _render_climate_settings():
    ...
    with c_temp1:
        st.session_state.project_settings["target_temp_heat"] = st.slider(
            "Цільова темп. (Опалення) °C", 10.0, 25.0,
            st.session_state.project_settings.get("target_temp_heat", 20.0), 0.5,
        )
    with c_temp2:
        st.session_state.project_settings["target_temp_cool"] = st.slider(
            "Цільова темп. (Охолодження) °C", 18.0, 30.0,
            st.session_state.project_settings.get("target_temp_cool", 24.0), 0.5,
        )
    st.session_state.project_settings["hvac_power_limit"] = st.number_input(
        "Макс. потужність кондиціонера (Вт)", 500, 20000,
        st.session_state.project_settings.get("hvac_power_limit", 3000), 500,
    )
    st.markdown("---")
...
def _calculate_mathematical_model(room_l, room_w, room_h, room_l_cut, room_w_cut):
...
def _plot_results(df_res, temp_dir):
    ...
    with tab_air:
        if "Generic Contaminant" in df_plot.columns:
            ...
            fig_air_zoom = px.line(
                df_plot, x=x_data, y="Generic Contaminant",
...
def _run_energyplus_simulation(room_l, room_w, room_h, room_l_cut, room_w_cut):
...
def render(room_l, room_w, room_h, room_l_cut, room_w_cut):
    _render_climate_settings()
    _calculate_mathematical_model(room_l, room_w, room_h, room_l_cut, room_w_cut)
    ...
    if run_ai:
        if _run_ai_optimization(room_l, room_w, room_h, room_l_cut, room_w_cut):
            run_sim = True
